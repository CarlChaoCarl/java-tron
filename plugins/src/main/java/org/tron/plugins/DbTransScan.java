...
@CommandLine.Command(name = "trans-scan",
    description = "scan data from trans.",
    exitCodeListHeading = "Exit Codes:%n",
    exitCodeList = {
        "0:Successful",
        "n:query failed,please check toolkit.log"})
public class DbTransScan implements Callable<Integer> {
  @CommandLine.Spec
  CommandLine.Model.CommandSpec spec;
  @CommandLine.Parameters(index = "0",
      description = " db path for trans")
  private Path db;
  @CommandLine.Option(names = {"-h", "--help"}, help = true, description = "display a help message")
  private boolean help;
  private static final  String DB = "trans";
  ...
  private int query() throws RocksDBException, IOException , SQLException {
    ClickHouseDataSource dataSource = new ClickHouseDataSource("jdbc:clickhouse://3.223.160.185:8123");
    ...
    try (DBInterface database  = DbTool.getDB(this.db, DB);
      ...
      for (iterator.seekToFirst(); iterator.hasNext(); iterator.next(), c++) {
        try{
          byte[] transBytes = iterator.getValue();
          Protocol.Transaction transaction = Protocol.Transaction.parseFrom(iterator.getValue());
        } catch (Exception e) {
          continue;
        }

          insertIntoClickHouse(iterator.getValue(), stat);
          spec.commandLine().getOut().format("current trans size: %d", c).println();
          break;
      }
      spec.commandLine().getOut().format("total trans size: %d", c).println();
      logger.info("total trans size: {}", c);
    } catch (Exception e) {
  ...
  private  void insertIntoClickHouse(byte[] v, ClickHouseStatement stat) {
    ...
    try {
// 数据插入
      Protocol.Transaction transaction = Protocol.Transaction.parseFrom(v);
      Map<String, String> stringStringMap = this.transaction2Map(transaction);
  ...
  private Map<String, String> transaction2Map(Protocol.Transaction transaction) throws InvalidProtocolBufferException {
...
*/
  ...
  public static void main(String[] args) {
    int exitCode = new CommandLine(new DbTransScan()).execute(args);
